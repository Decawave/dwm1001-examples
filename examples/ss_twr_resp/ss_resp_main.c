 ...
 *           then sends a "response" message recording its TX time-stamp. See Note 1 below on limitations of this simple example. 
 *
...
/* Inter-ranging delay period, in milliseconds. See NOTE 2*/
#define RNG_DELAY_MS 1000

/* Frames used in the ranging process. See NOTE 3,4 below. */
...
// See note 7 at the end of this file
...
int ss_resp_run(void)
{
        ...
        if (status_reg & SYS_STATUS_RXFCG)
        {
            ...
            if (memcmp(rx_buffer, rx_poll_msg, ALL_MSG_COMMON_LEN) == 0)
            {
                ...
                /* Write all timestamps in the final message. See NOTE 6 below. */
                resp_msg_set_ts(&tx_resp_msg[RESP_MSG_POLL_RX_TS_IDX], poll_rx_ts);
                resp_msg_set_ts(&tx_resp_msg[RESP_MSG_RESP_TX_TS_IDX], resp_tx_ts);

                /* Write and send the response message. See NOTE 7 below. */
 ...
 * 5. dwt_writetxdata() takes the full size of the message as a parameter but only copies (size - 2) bytes as the check-sum at the end of the frame is
 ...
 *    bytes.
 * 7. POLL_RX_TO_RESP_TX_DLY_UUS is a critical value for porting to different processors. For slower platforms where the SPI is at a slower speed 
 ...
 *    system(processor).
 *    The high order byte of each 40-bit time-stamps is discarded here. This is acceptable as, on each device, those time-stamps are not separated by
